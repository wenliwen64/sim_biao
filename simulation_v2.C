...
int simulation_v2(int argc=0, int particlenum = 500, int eventsnum = 100000)
{
    ...
    //=============gamma correlator============
    TProfile* hgamma_rp = new TProfile("gamma_rp", "gamma_rp", 6, 0.5, 6.5);
    TProfile* hgamma_west_ep = new TProfile("gamma_west_ep", "gamma_west_ep", 6, 0.5, 6.5);
    TProfile* hgamma_east_ep = new TProfile("gamma_east_ep", "gamma_east_ep", 6, 0.5, 6.5);
	...
	//**************Full event used to resconstruct charge separation signal*****************************************
	int iflag0 = 1;
	...
	//********************East data used to reconstruct charge separation*********************************
	int iflag1=1;
	...
	if(iflag1==1)
	{
        ...
        //**********************West data used to recosntruct charge separation signal**************************************************
	int iflag2=1;
	...
	hv2[1]->Fill(v2full_ep);     
        ...
        //===============Compute the gamma Corelator===============================
        int npairs_gamma_ss_rp = 0;
        int npairs_gamma_os_rp = 0;
        double gamma_ss_rp = 0;
        double gamma_coscos_ss_rp = 0;
        double gamma_sinsin_ss_rp = 0;
        double gamma_os_rp = 0;
	...
	for(int i = 0; i < nn; i++){
            for(int j = 0; j < nn; j++){
                ...
                if(Charge[i] == Charge[j]){
                    gamma_ss_rp += cos(Phi[i] + Phi[j]);
                    gamma_coscos_ss_rp += cos(Phi[i]) * cos(Phi[j]);
		    gamma_sinsin_ss_rp += sin(Phi[i]) * sin(Phi[j]);

                    npairs_gamma_ss_rp++;
		}
                else{
                    gamma_os_rp += cos(Phi[i] + Phi[j]); 
                    gamma_coscos_os_rp += cos(Phi[i]) * cos(Phi[j]);
                    gamma_sinsin_os_rp += sin(Phi[i]) * sin(Phi[j]);
                    npairs_gamma_os_rp++;
		}
	    }
	}

        gamma_ss_rp /= npairs_gamma_ss_rp;
        gamma_coscos_ss_rp /= npairs_gamma_ss_rp;
        gamma_sinsin_ss_rp /= npairs_gamma_ss_rp;

        gamma_os_rp /= npairs_gamma_os_rp;
	...
	std::vector<int> Charge_wrt_east_ep;
        std::vector<double> Phi_west_ep;
        std::vector<double> Phi_east_ep;
	for(int i = 0; i < nn; i++){
	...
	ss_in_rp /= npairs_msc_ss_rp;
	ss_out_rp /= npairs_msc_ss_rp;
	os_in_rp /= npairs_msc_os_rp;
	os_out_rp /= npairs_msc_os_rp;
	...
	//===============Compute the MSC/Gamma Correlator: east data, west ep==============================
	...
	double os_out_west_ep = 0;
        ...
        double gamma_ss_west_ep = 0;
        double gamma_coscos_ss_west_ep = 0;
        double gamma_sinsin_ss_west_ep = 0;
        double gamma_os_west_ep = 0;
        double gamma_coscos_os_west_ep = 0;
        double gamma_sinsin_os_west_ep = 0;
